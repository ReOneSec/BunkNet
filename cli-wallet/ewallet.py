...
def get_address_info(address: str):
    """Gets full address details (balance, nonce, txs) from the API."""
    try:
...
def send_transaction(key_pair: SigningKey, recipient: str, amount: float, fee: float, nonce: int):
    """Builds, signs, and sends a transaction with the correct nonce."""
    ...
    try:
        ...
        print(f"   Tx ID: {response.json().get('transaction_id')}")
    except requests.exceptions.RequestException as e:
...
def handle_manage_wallet():
    ...
    if os.path.exists(WALLET_FILE):
        ...
        if overwrite != 'yes': print("Operation cancelled."); return
    ...
    if choice == 'g':
        ...
        print(f"{Colors.RED}{Colors.BOLD}🛑 IMPORTANT: Write this phrase down and store it securely.{Colors.ENDC}")
...
def handle_balance():
    ...
    print(f"{Colors.CYAN}   Next Nonce: {info.get('nonce', 0)}{Colors.ENDC}")
...
def handle_send():
    ...
    print(f"Your current balance is {balance:.4f} $BUNK. Your next transaction must use nonce {nonce}.")
    ...
    try:
        ...
        fee = float(input(f"Enter transaction fee (default 0.01): ") or "0.01")
    except ValueError:
        print(f"{Colors.RED}❌ Invalid input. Please check the address and numbers.{Colors.ENDC}")
        return

    if (amount + fee) > balance:
        print(f"{Colors.RED}❌ Error: Insufficient funds.{Colors.ENDC}")
        return
    ...
    if confirm.lower() != 'yes':
        print("Transaction cancelled."); return
    
    print(f"\n{Colors.BLUE}Sending transaction...{Colors.ENDC}")
    send_transaction(key_pair, recipient, amount, fee, nonce)
...
"""
    ...
    print(f" {Colors.BLUE}3.{Colors.ENDC} View Balance & Nonce")
...
def main():
    while True:
        ...
        if choice == '1': handle_manage_wallet()
        elif choice == '2': handle_address()
        elif choice == '3': handle_balance()
        elif choice == '4': handle_backup()
        elif choice == '5': handle_send()
        elif choice == '6': handle_history()
        elif choice == 'q': print("Goodbye!"); break
        else: print(f"{Colors.RED}Invalid choice, please try again.{Colors.ENDC}")
